(function () {
    'use strict';
    angular.module("bp.core").factory("GameService", GameService);
    GameService.$inject = ['$http'];
    function GameService($http) {
        function GetStats(username, outcome, matchup) {
            var url = "/odata/GameResults?$filter=userName eq '" + username + "' and outcome eq NeshHouse.Stats.Web.Models.GameOutcome'" + outcome + "' and game/matchup eq NeshHouse.Stats.Web.Models.Matchup'" + matchup + "'";
            return $http.get(url);
        }
